...
int Do_prepare(void *buff, uint16_t addr)
{
    framebuff_t fb;
...
int Do_parse(uint32_t senid, void *strbuf, void *datbuf, uint32_t size)
{
    ...
    sprintf (strbuf, "%d|%d|%.3f", senid, rtc_get_time(), Do);
    FRAMEBUFF_PARSE_END();
    
    return strlen(strbuf);
}

struct senproto_struct Do_senproto = {Do_prepare, Do_parse};
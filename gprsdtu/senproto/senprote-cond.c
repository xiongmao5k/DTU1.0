...
int cond_prepare(void *buff, uint16_t addr)
{
    framebuff_t fb;
...
int cond_parse(uint32_t senid, void *strbuf, void *datbuf, uint32_t size)
{
    ...
    sprintf (strbuf, "%d|%d|%f", senid, rtc_get_time(), cond);
    FRAMEBUFF_PARSE_END();
    
    return strlen(strbuf);
}

struct senproto_struct cond_senproto = {cond_prepare, cond_parse};